...
function refreshSession(options, callback) {
    ...
    request.post(
        ...
        (err, response, body) => {
            const error = compileError(
                "Error on AgentVEP RefreshSession",
                err,
                response,
                body,
                AUTH_SERVICE_CONTEXT.REFRESH_SESSION
            );
            callback(error, body);
        }
    );
...
function compileError(baseErrorMessage, err, response, body, serviceContext) {
    ...
    if (response && response.statusCode > 299) {
        return new SDKError(
            `${baseErrorMessage}: ${response.statusMessage}`,
            response.statusCode,
            serviceContext
        );
    }
    ...
    if (body && body.error) {
        return new SDKError(
            `${baseErrorMessage}: ${JSON.stringify(body.error)}`,
            body.error.internalCode,
            serviceContext
        );
    }
