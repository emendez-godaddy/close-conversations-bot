...
class SDK extends Events {
    constructor(conf) {
        ...
        //load skillIds with corresponding time in hours for conversation closing purposes
        this.closingConvoTimeBySkill = CONST.CLOSINGCONVOTIMEBYSKILL;
    ...
    connect(connectCallback) {
        async.waterfall(
            [
                ...
                // 2. login
                (domains, callback) =>
                    this._handleLogin(domains, "Connect#Login", callback),
            ],
            (err, token, domains) => {
                if (err) {
                ...
                const wsDomain =
                    this.conf.wsDomainType && domains[this.conf.wsDomainType]
                        ? domains[this.conf.wsDomainType]
                        : domains.facadeMsg;
                this._init(
                    Object.assign(
                        {
    ...
    reconnect(dontRegenerateToken) {
        ...
        this.csdsClient.getAll((err, domains) => {
            const domainsError = !domains
                ? new SDKError(
                      "could not fetch domains on reconnect",
                      null,
                      SERVICES.DOMAIN_NAMES
                  )
                : null;
        ...
        async.waterfall(
            [
                ...
                (domains, callback) => {
                    ...
                    this._handleRefreshSessionCall(
                        domains,
                        "RefreshSession#REST",
                        (err, domains) => {
        ...
        external.login(loginData, (err, data, cookies) => {
            ...
            // if userId is undefined then we need to throw an error
                ...
                if (typeof data === "undefined") data = "[undefined]";
                    ...
                    if (data.hasOwnProperty("accountData"))
                        delete data.accountData;
                    if (data.hasOwnProperty("csdsCollectionResponse"))
                        delete data.csdsCollectionResponse;
                }
                // console.warn(`invalid login state, userId is undefined, data: ${JSON.stringify(data)}`);

                // then emit an error
                let err = new SDKError(
                    "invalid login state, userId is undefined",
                    null,
                    AUTH_SERVICE_CONTEXT.LOGIN
                ); // make sure to have whole response - data
                return callback(err, null, domains);
            }
    ...
    _createAndSendRequest(
        ...
        if (Transformer[type]) {
            ...
            ({ body, type, headers, metadata, encodedMetadata } = Transformer[
                type
            ]({ body, type, headers, metadata, encodedMetadata }, this));
        }
    ...
    _preCheckRefreshSession() {
        ...
        const error = isCSRForJarUndefined
            ? new SDKError(
                  "CSRF and JAR are not defined for this agent. Please reconnect with bearer token.",
                  null,
                  AUTH_SERVICE_CONTEXT.LOGIN
              )
            : null;
    ...
    _handleSocketCreated(data) {
        ...
        this.emit(
            ...
            data || { connected: true, ts: Date.now() }
        );
    }
    ...
    _queueForResponseAndSend(request, callback) {
        if (request && this.connected) {
            ...
            callback(
                new SDKError("socket not connected", null, SERVICES.MESSAGING)
            );
        }
    }
    ...
    _checkForErrors() {
        ...
        for (let key in this.pendingRequests) {
            //Check for requests taking too long
            if (
                this.pendingRequests.hasOwnProperty(key) &&
                this.pendingRequests[key].launchTime
            ) {
                const timeElapsed = now - this.pendingRequests[key].launchTime;
                if (timeElapsed > this.pendingRequests[key].timeout) {
        ...
        if (timedOutCallbacks.length) {
            for (let i = 0; i < timedOutCallbacks.length; i++) {
    ...
    _handleClosed(data) {
        ...
        this.emit(
            CONST.EVENTS.CLOSED,
            data || { connected: false, ts: Date.now() }
        );
    }
